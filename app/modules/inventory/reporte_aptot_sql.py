...
def _sql_text_as_bigint(expr: str) -> str:
    """Convierte texto a bigint solo si es numérico (UNION con itemcards)."""
    return f"""CASE
        WHEN TRIM(COALESCE({expr}::text, '')) ~ '^[0-9]+$'
        THEN TRIM({expr}::text)::bigint
        ELSE NULL
    END"""
...
# Las columnas de inventario se completan con datos de Margesi (no hay hoja de captura).
...
SELECT
    ...
    m.inv_num_1::text,
    m.inv_num_2::text,
    {_sql_text_as_bigint("m.inv_num")} AS inv_num,
    NULL AS item_created_at,
    NULL AS item_updated_at,
    {_sql_text_as_bigint("m.inv_hoj")} AS hoj_num,
    NULL AS hoj_fec,
    COALESCE(cct.code, m.cct_cod) AS area_code,
    cct.description AS area_description,
    COALESCE(env.code, m.amb_cod) AS ambiente_code,
    env.description AS ambiente_description,
    ...
    COALESCE(NULLIF(TRIM(pe.number), ''), m.usu_cod) AS usuario_code,
    {_PERSON_USUARIO_PE} AS usuario,
    m.mar_cont_fec,
    m.mar_cont_doc,
    m.mar_cont_cta,
    m.mar_cont_val,
...
SELECT
    ...
    m.mar_ncha,
    m.mar_obs,
    m.inv_num_1::text AS inv_num_1,
    m.inv_num_2::text AS inv_num_2,
    {_sql_text_as_bigint("m.inv_num")} AS inv_num,
    NULL AS item_created_at,
    NULL AS item_updated_at,
    {_sql_text_as_bigint("m.inv_hoj")} AS hoj_num,
