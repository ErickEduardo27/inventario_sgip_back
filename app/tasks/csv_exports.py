...
@celery_app.task(bind=True, name="export.margesi_csv")
def export_margesi_csv_task(
    self,
    job_id: str,
    tenant_id: str,
    query_dict: dict,
    ...
    try:
        ...
        inner_sql, params, filename_base = build_margesi_export_query(tenant_uuid, q)
        stamp = date.today().isoformat()
        payload = copy_query_to_csv_bytes(inner_sql, params)
        if fmt == "xlsx":
            self.update_state(state="PROGRESS", meta=_progress_meta(45, "Convirtiendo a Excel…"))
            content = csv_bytes_to_xlsx_bytes(payload)
            filename = f"{filename_base}_{stamp}.xlsx"
        else:
            content = b"\xef\xbb\xbf" + payload
            filename = f"{filename_base}_{stamp}.csv"
