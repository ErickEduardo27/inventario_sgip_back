"""Módulo UI Reporte APTOT por locales (sidebar + permisos por perfil).

Revision ID: 047_reporte_aptot_locales_module
Revises: 046_enviroments_reporte
"""
...
revision = "047_reporte_aptot_locales_module"
down_revision = "046_enviroments_reporte"
branch_labels = None
depends_on = None

REPORTE_APTOT_LOCALES_COMPONENT = {
    "code": "reporte_aptot_locales",
    "name": "APTOT por locales",
    "group_name": "Reportes",
    "route": "/reporte-aptot-locales",
    "icon": "Building2",
    "order_index": 23,
}
...
def upgrade() -> None:
    conn = op.get_bind()
    comp = REPORTE_APTOT_LOCALES_COMPONENT
    row = conn.execute(
...
def downgrade() -> None:
    ...
    comp_row = conn.execute(
        sa.text("SELECT id FROM ui_components WHERE code = :code"),
        {"code": REPORTE_APTOT_LOCALES_COMPONENT["code"]},
    ).fetchone()
    if comp_row:
        conn.execute(
